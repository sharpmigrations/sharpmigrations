...
namespace Sharp.Data {
    public class Database : IDatabase {
        ...
        public int ExecuteSql(string call, params object[] parameters) {
            try {
                if (!String.IsNullOrEmpty(Provider.ExecuteThisParameterlessSqlBeforeAnyOther())) {
                    TryExecuteSql(Provider.ExecuteThisParameterlessSqlBeforeAnyOther(), new object[] { });
                }
                return TryExecuteSql(call, parameters);
            } catch (Exception ex) {
                if (!String.IsNullOrEmpty(Provider.ExecuteThisParameterlessSqlAfterRaiseAnException())) {
                    TryExecuteSql(Provider.ExecuteThisParameterlessSqlAfterRaiseAnException(), new object[] { });
                }
